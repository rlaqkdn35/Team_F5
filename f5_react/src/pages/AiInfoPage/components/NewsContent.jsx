...
for (let i = 1; i <= 20; i++) {
  mockNewsData.push({
    ...
    url: `#news-detail-${i}` // 실제 뉴스 상세 URL
  });
}
  ...
  return (
    <div className="news-content-page">
      ...
      <div className="news-list-container-nc">
        {newsItems.map(news => (
          <a href={news.url} target="_blank" rel="noopener noreferrer" key={news.id} className="news-item-card-nc">
      ...
      {/* 20개 이상일 경우 페이지네이션 또는 '더보기' 버튼 필요 */}
    </div>
  );
};
